  ...
  def __init__(self, path=datapath, 
    ...
    self.transform = transforms.Compose([
           ...
           # transforms.CenterCrop(self.cropsize),
            transforms.Resize((self.resize,self.resize),interpolation=Image.LANCZOS),
            transforms.ToTensor()
        ])
...
if __name__ == '__main__':
    traindata = VideoDataset(load_from_tensorfile=False, 
        windowsize=2, 
        save_to_tensorfile=True, 
        tensorfile_prefix='multicar_zoomout_224_train',
        names=trainvids)
    print(len(traindata))
    testdata = VideoDataset(load_from_tensorfile=False,
        windowsize=2,
        save_to_tensorfile=True,
        tensorfile_prefix='multicar_zoomout_224_test',
        names=testvids
        )
    print(len(testdata))
    # print(d.numVideos())
